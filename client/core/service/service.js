...
class Injector {
  ...
  getComponentsToUpdate(calledBy) {
    return this.components.filter(component => component.toRender.indexOf(calledBy) !== -1);
  }

  updateComponents(calledBy) {
    const componentsToUpdate = this.getComponentsToUpdate(calledBy);

    return Promise.all(componentsToUpdate.map(component => new Promise((resolve) => {
  ...
  createInstance(service) {
    ...
    const methods = Helpers.getDescriptors(service);
  ...
  get(arrayFormat = false) {
    if (arrayFormat) { return this.services.map(service => service.instance); }
